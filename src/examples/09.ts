...
export default async function run() {
  ...
  console.log(john === john2, JSON.stringify(john2.data)); // false {"id":1,"name":"John","age":"20"}
  ...
  console.log(models4.length, models4.includes(john2), models4.includes(jane), models4.includes(sam)); // 2 true false false

  // загружаем все заново
  const models5 = await coll.find();

  // так как флаг reload по умолчанию выставлен, то все экземпляры будут перезаписаны
  console.log(models4.map(m => m.instanceDestroyed)); // [true, true]
